...
1. Group marks for all graded activities assigned by me (the professor): $G_{H1}, G_{H2}, G_{E1}, G_{E2}, G_{EF}.$

1. Individual extra marks for partial exams and final exam only (earned through individual student effort): $X_{E1}, X_{E2}, X_{EF}.$

1. Coevaluations from teammates for homeworks and partial exams (peer assessment): $C_{H1}, C_{H2}, C_{E1}, C_{E2}.$

1. Exams (group mark + individual extra mark):  
$$
\begin{align}
GX_{E1} &= G_{E1} + X_{E1}, \\
GX_{E2} &= G_{E2} + X_{E2}.
...
1. Apply coevaluations for homework assignments $H_1$ and $H_2$:
$$
\begin{aligned}
...
\end{aligned}
$$
6. Apply coevaluations for partial exams $E_1$ and $E_2$:
...
Where $E_F = G_{EF} + X_{EF}$, and no coevaluation is applied to $E_F$.
...
1. See [DataCamp policies](#lst:dcp).
